...
function pushedButton(emne, beskrivelse, links){
    if($('#wrapper .border-popup').length <= 0){
        ...
        $('#wrapper .border-popup').append('<h4></h4>');
        $('#wrapper .border-popup').append('<p></p>');
        $('#wrapper .border-popup').append('<p></p>');
    }
     $('#wrapper .border-popup h4').text(emne);
     $('#wrapper .border-popup p:eq(0)').text(beskrivelse); 
    var linkText = '';
    for(var i = 0; i < links.length; i++){
        linkText += '<a href="' + links[i] + '">' + links[i] + '</a></br>';
    }
    console.log(links.length);
     $('#wrapper .border-popup p:eq(1)').html(linkText);
