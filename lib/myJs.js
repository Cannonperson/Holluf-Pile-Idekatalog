...
function klasser(){
       ...
       container1.addEventListener('click', function (e) {createjs.Tween.get(container1).to({alpha:1,scaleX:0.5,scaleY:0.5,rotation:0,x:100,y:100}, 400).wait(400).to({visible:false});});	 
...
function loadDataFromJson(selectedClass){
    ...
    for(var i = 0; i < data.classes.length; i++){
        ...
        for(var i2 = 0; i2 < data.classes[i].emner.length; i2++){
            ...
            $('#overEmne'+overEmneIndex).append('<div><button class="emneButton" id="emne'+i +''+i2+'">' + data.classes[i].emner[i2].emneOverskrift +'</button></div>');
...
$(function(){
    $('.emneButton').on('click',function(e){
        console.log(e.target.id.substring(4,5));
        var classIndex = e.target.id.substring(4,5);
        var emneIndex = e.target.id.substring(5);
        var s1 = Singleton.getInstance();
        //pushedButton(data.classes[classIndex].emner[emneIndex].emneOverskrift, data.classes[classIndex].emner[emneIndex].emneBeskrivelse, data.classes[classIndex].emner[emneIndex].links)
    });
});
}